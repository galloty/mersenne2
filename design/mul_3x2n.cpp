...
{
	const GF r2s = GF::root_nth(2 * s);

	for (size_t j = 0; j < s; ++j)
	{
		const GF wj = r2s.pow(bitrev(j, s));
		for (size_t i = 0; i < m; ++i)
		{
			const size_t k = 2 * m * j + i;
			const GF u0 = x[k + 0 * m], u1 = x[k + 1 * m] * wj;
...
{
	const GF r2si = GF::root_nth(2 * s).invert();

	for (size_t j = 0; j < s; ++j)
	{
		const GF wji = r2si.pow(bitrev(j, s));
...
{
	const GF J2 = J * J;
	const GF r3m = GF::root_nth(3 * m);

	for (size_t i = 0; i < m; ++i)
	{
		const GF u0 = x[i + 0 * m], u1 = x[i + 1 * m], u2 = x[i + 2 * m];
		x[i + 0 * m] = (u0 +       u1 +      u2);
		x[i + 1 * m] = (u0 +   J * u1 + J2 * u2) * r3m.pow(1 * i);
		x[i + 2 * m] = (u0 +  J2 * u1 +  J * u2) * r3m.pow(2 * i);
	}
}
...
{
	const GF J2 = J * J;
	const GF r3mi = GF::root_nth(3 * m).invert();

	for (size_t i = 0; i < m; ++i)
	{
		const GF u0 = x[i + 0 * m], u1 = x[i + 1 * m] * r3mi.pow(1 * i), u2 = x[i + 2 * m] * r3mi.pow(2 * i);
		x[i + 0 * m] = (u0 +      u1 +      u2) * inv3;
		x[i + 1 * m] = (u0 + J2 * u1 +  J * u2) * inv3;
		x[i + 2 * m] = (u0 +  J * u1 + J2 * u2) * inv3;
	}
}
...
int main()
{
	...
	forward3(R, m);
	square2(&R[0 * m], m); square2(&R[1 * m], m); square2(&R[2 * m], m);
	backward3(R, m);
